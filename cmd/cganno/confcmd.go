...
import (
	...
	"github.com/compgenlab/cganno/internal/prompt"
)
...
func cmdInit(cfgPath string, args []string) error {
	...
	cfg := config.Config{
		DataDir:         "$CGANNO_HOME/data",
		CacheDir:        "$CGANNO_HOME/data/cache",
		DefaultSnapshot: snap,
		AnnotationsDir:  annDir,
		Database:        config.Database{Backend: "sqlite", Path: "$CGANNO_HOME/cganno.db"},
		// Reference FASTAs are keyed by assembly; the starter snapshot is GRCh38.
		References: map[string]config.Reference{"GRCh38": {Fasta: "$CGANNO_HOME/ref/GRCh38.fa"}},
	}
	...
	// Real sources/tools are added later via `cganno configure` or `cganno registry`.
	...
	// real sources and choose their default annotations via `cganno configure`.
	...
	// piped/non-interactive `cganno init` must not try to launch it.
	if stdinIsTerminal() && pr.AskBool("configure annotation sources & tools now?", true) {
		return cmdEdit(cfgPath, nil)
	}
	fmt.Println("run `cganno configure` to add sources/tools, then `cganno download` to fetch them")
	return nil
}
